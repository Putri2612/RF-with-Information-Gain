...
if selected == 'Testing':
    ...
    if st.button("Uji Sentimen"):
       ...
       # Memuat data dan model TF-IDF dari fungsi load_data
        ...
        # Load model Random Forest dan objek TF-IDF yang digunakan saat melatih model
        model = joblib.load('best_rf_model_smote_tiga.pkl')
        # Buat objek TfidfVectorizer dengan max_features = 841
        tfidf = TfidfVectorizer(max_features=841)
        # Mengambil X_selected dari session
        X_selected = st.session_state.X_selected
        # Lakukan vektorisasi pada data yang telah melalui SMOTE dan seleksi fitur
        x_smote_feature_selected = X_selected # Gantilah ini dengan data yang sesuai

        x_tfidf = tfidf.fit_transform(x_smote_feature_selected)

        # Vektorisasi teks baru menggunakan objek TF-IDF yang telah Anda muat kembali
        new_data_vector = tfidf.transform(new_data)

        # Melakukan prediksi dengan model terlatih
        ...
        # Check the dimensions of input_tfidf
